...
# Create a new column to categorize years into blocks
delay_summary <- delay_summary %>%
  mutate(period = ifelse(year >= 2020 & year <= 2021, "COVID (2020-2021)", "Post-COVID (2022-2024)"))

# Create a bar chart
ggplot(delay_summary, aes(x = line, y = total_delays, fill = period)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Total Delays by Subway Line",
       x = "Subway Line",
       y = "Total Delays",
       fill = "Period") +
  theme_minimal() +
  scale_x_discrete(drop = FALSE)  # Ensure all lines are displayed
...
# Optionally, print message to confirm save
cat("Animation saved as", output_gif_path)
