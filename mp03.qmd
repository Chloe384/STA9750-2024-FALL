...
# Assuming all_years_data is already created, now let's plot it with adjusted axes.
...
ggplot(data = all_years_data) +
  ...
  geom_sf_text(aes(label = EC, geometry = geom), color = "black", size = 3, fontface = "bold") +  # Add EC vote labels
  theme_minimal() +
  labs(title = "U.S. Presidential Election Results by Year", subtitle = "Winning Party by State and EC Votes") +  # Added subtitle
  theme(
    ...
    axis.title = element_blank()  # Remove axis labels (titles)
  ) +
  facet_wrap(~ year, ncol = 1)  # Facet by year, arranging all years in a single column
